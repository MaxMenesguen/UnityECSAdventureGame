...
public readonly partial struct ProjectileAspect : IAspect
{
    ...
    public void Move(float time, ComponentLookup<LocalToWorld> positions)
    {
        ...
        // Ensure forward direction is correctly calculated
        float3 forward = math.mul(_transform.ValueRO.Rotation, new float3(0, 0, 1)); // Forward is along the local Z-axis

        // Apply movement
        _transform.ValueRW.Position += _speed.ValueRO.value * time * forward;
